...
d <- dataSIM[,c(1:6)]
image(c(1:dim(d)[2]),c(1:dim(d)[1]),t(d),xlab="condition",ylab="genes",col=greenred(75))
par(fig=c(0.25,1,0,1), new=TRUE,xpd=TRUE)
d <- dataSIM[,-c(1:6)]
#d <- dataMFA
image(c(1:dim(d)[2]),c(1:dim(d)[1]),t(d),xlab="condition",ylab="genes",col=greenred(75),axes=FALSE)
@
...
@
<<cheat1,echo=FALSE>>=
#save(list="out_MFA",file="figure/out_MFA.RData")
#save(list="out_ZG",file="figure/out_ZG.RData")
...
<<CSpermute_plots,echo=TRUE,eval=TRUE,cache=TRUE,message=FALSE,results='hide',fig.keep="high",fig.show='hold',fig.pos='H',out.width='9cm',out.height='10cm',fig.align='left',fig.cap="CSpermute graphs for out_MFA">>=
